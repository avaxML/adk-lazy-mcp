...
from .config import RegistryConfig, RetrievalConfig, ServerConfig
...
class ToolSchema(BaseModel):
    ...
    retrieval: RetrievalConfig = Field(default_factory=RetrievalConfig, exclude=True, repr=False)
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        self.search_terms = tuple(
            _build_document_terms(self.name, self.description, self.input_schema, self.retrieval)
        )
...
class CatalogManager:
    def __init__(self, registry_config: RegistryConfig) -> None:
        ...
        self._retrieval = registry_config.retrieval
    ...
    def register_server(self, server: ServerConfig) -> None:
        ...
        self._lexical_indexes[server.name] = _ServerLexicalIndex.empty(self._retrieval)
    ...
    async def hydrate_server(self, server: str, tools: list[dict[str, Any]]) -> None:
        ...
        lexical_index = _ServerLexicalIndex.build(server, mapped, self._retrieval)
        semantic_vectors = {
            name: _build_semantic_vector(
                (*_tokenize_identifier(server), *tool.search_terms), self._retrieval
            )
            for name, tool in mapped.items()
        }
        tool_families = _build_tool_families(mapped, self._retrieval)
    ...
    def _semantic_fallback_matches(
        ...
        for rank, (tool_name, score) in enumerate(
            ...
            if (
                score < self._retrieval.semantic_fallback_threshold
                and name_similarity < self._retrieval.semantic_name_fallback_threshold
            ):
                continue
            matches.append(
                _SearchMatch(
                    ...
                    description=entry.tools[tool_name].description,
                    family=families.get(tool_name, tool_name),
                    score=_reciprocal_rank_fusion_score(rank, self._retrieval),
                )
            )
        return matches
...
class _ServerLexicalIndex:
    ...
    @classmethod
    def empty(cls, retrieval: RetrievalConfig) -> _ServerLexicalIndex:
        return cls(
            doc_terms={},
            doc_lengths={},
            doc_freq=Counter(),
    ...
    @classmethod
    def build(
        cls, server: str, tools: dict[str, ToolSchema], retrieval: RetrievalConfig
    ) -> _ServerLexicalIndex:
    ...
    def search(self, query: str, tools: dict[str, ToolSchema]) -> list[tuple[str, float]]:
        ...
        for tool_name, term_freqs in self.doc_terms.items():
            ...
            for term in query_terms:
                ...
                norm = 1.0 - self.bm25_b + self.bm25_b * (doc_len / max(self.avg_doc_len, 1.0))
                score += idf * ((freq * (self.bm25_k1 + 1.0)) / (freq + self.bm25_k1 * norm))
...
def _build_document_terms(
    name: str, description: str, schema: dict[str, Any], retrieval: RetrievalConfig
) -> list[str]:
    name_terms = _tokenize_identifier(name)
    description_terms = _extract_alphanumeric_tokens(description)
    schema_terms = _schema_terms(schema, retrieval)
    return [*(name_terms * retrieval.name_term_weight), *description_terms, *schema_terms]


def _schema_terms(schema: Any, retrieval: RetrievalConfig) -> list[str]:
    tokens: list[str] = []
    ...
    if isinstance(properties, dict):
        for prop_name, prop_schema in properties.items():
            ...
            tokens.extend(prop_terms * retrieval.schema_property_weight)
            tokens.extend(_schema_terms(prop_schema, retrieval))
    ...
    if isinstance(required, list):
        for item in required:
            if isinstance(item, str):
                ...
                tokens.extend(required_terms * retrieval.required_field_weight)

    items = schema.get("items")
    if isinstance(items, dict):
        tokens.extend(_schema_terms(items, retrieval))
    ...
    for key in ("allOf", "anyOf", "oneOf"):
        ...
        if isinstance(value, list):
            for item in value:
                tokens.extend(_schema_terms(item, retrieval))
    return tokens
...
def _build_semantic_vector(
    token_sequence: Iterable[str], retrieval: RetrievalConfig
) -> dict[str, float]:
    ...
    for token in token_sequence:
        ...
        if len(token) < retrieval.trigram_size:
            continue
        padded = f"^{token}$"
        for idx in range(len(padded) - (retrieval.trigram_size - 1)):
            features[f"tri:{padded[idx : idx + retrieval.trigram_size]}"] += (
                retrieval.trigram_weight
            )
...
def _build_tool_families(
    tools: dict[str, ToolSchema], retrieval: RetrievalConfig
) -> dict[str, str]:
    leaders: list[tuple[str, set[str]]] = []
    assignments: dict[str, str] = {}
    for tool in sorted(tools.values(), key=lambda item: item.name):
        signature = _cluster_signature(tool, retrieval)
        ...
        if best_similarity >= retrieval.leader_cluster_threshold:
...
def _cluster_signature(tool: ToolSchema, retrieval: RetrievalConfig) -> set[str]:
    stopwords = frozenset(retrieval.cluster_stopwords)
    return {
        token
        for token in tool.search_terms
        if len(token) >= retrieval.min_cluster_token_length and token not in stopwords
    }
...
def _reciprocal_rank_fusion_score(rank: int, retrieval: RetrievalConfig) -> float:
    return 1.0 / (retrieval.reciprocal_rank_k + rank)
